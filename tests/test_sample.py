...
def _load_scan_module():
    ...
    assert spec.loader is not None, "spec.loader é None — loader ausente"
...
def test_scanner_module_integrity():
    ...
    assert hasattr(module, "main"), "O script deve expor main()"
    assert hasattr(module, "run_iac_scan"), "Deve expor run_iac_scan()"
    assert hasattr(module, "run_ruff"), "Deve expor run_ruff()"
    ...
    assert has_pip_audit, "Função para pip-audit ausente (ex: run_pip_audit)"
...
def test_no_unexpected_side_effects_on_import():
    ...
    assert not suspicious_globals, "Módulo não deve executar scans na importação"